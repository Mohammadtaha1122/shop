...
function CalaDetail() {
  ...
  useEffect(() => {
    async function fetchResult() {
      ...
      setProducts(found)
      // setReview(data.products.reviews)
      // console.log(review)
    }
    fetchResult()
  } ,[])
    ...
    <div className="container mb-25">
      <div className=" container justify-between flex max-md:flex-col mt-1">
        ...
        <div className="mt-10 w-[550px] max-md:w-auto">
          ...
          <button onClick={() => setBuying(prev => [...prev ,products])} className="w-full rounded-xl text-white bg-orange-400 py-4 mt-6 transition ease-in hover:bg-orange-500 cursor-pointer">
            افزودن به سبد خرید
          </button>
        </div>
      </div>
